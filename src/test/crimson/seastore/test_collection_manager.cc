  ...
  run_async([this] {
    ...
    }
    {
      ...
      for (int i = 0; i < 20; i++) {
        ...
        [[maybe_unused]] auto ret = collection_manager->create(coll_root, *t, cid, coll_info_t(i)).unsafe_get0();
    ...
    checking_mappings(coll_root);
    {
      ...
      for (auto& ite : test_coll_mappings) {
       [[maybe_unused]] auto ret = collection_manager->remove(coll_root, *t, ite.first).unsafe_get0();
...
TEST_F(collection_manager_test_t, update)
{
  run_async([this] {
    ...
    }
    {
       ...
       auto t = tm->create_transaction();
       [[maybe_unused]] auto ret = collection_manager->update(coll_root, *t, iter1->first, iter2->second).unsafe_get0();
       tm->submit_transaction(std::move(t)).unsafe_get();
