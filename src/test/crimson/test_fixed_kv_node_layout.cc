...
TEST(FixedKVNodeTest, basic) {
  ...
  iter.set_lb(1);
  ASSERT_EQ(iter.get_lb(), 1);

  auto val = test_val_t{ 1, 1 };
  iter.set_val(val);
  ASSERT_EQ(val, iter.get_val());

  ASSERT_EQ(std::numeric_limits<uint32_t>::max(), iter.get_ub());
}
...
TEST(FixedKVNodeTest, at_capacity) {
  ...
  num = 0;
  for (auto &i : node) {
    ASSERT_EQ(i.get_lb(), num);
    ASSERT_EQ(i.get_val(), (test_val_t{num, num}));
    if (num < (CAPACITY - 1)) {
      ASSERT_EQ(i.get_ub(), num + 1);
    } else {
      ASSERT_EQ(std::numeric_limits<uint32_t>::max(), i.get_ub());
    }
    ++num;
  }
}
