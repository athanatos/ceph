...
TEST(pgtransaction, clone_safe_create_traverse)
{
  ...
  t.safe_create_traverse(
    [&](const pair<const hobject_t, PGTransaction::ObjectOperation> &p) {
      ...
      if (num == 0) {
	ASSERT_EQ(p.first, h);
	ASSERT_TRUE(boost::get<T::Create>(&p.second.init_type));
	ASSERT_TRUE(boost::get<T::Create>(&p.second.init_type)->clone_source);
	ASSERT_EQ(
	  *(boost::get<T::Create>(&p.second.init_type)->clone_source),
	  h2);
      } else if (num == 1) {
