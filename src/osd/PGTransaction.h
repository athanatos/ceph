 ...
 * 1) Each object included in the transaction must start with
 *    a creation operation (create, clone, rename, declare_exists)
 * 2) Rename sources may only be referenced prior to the rename
 *    operation to the destination.
 * 3) The graph formed by edges of source->destination for clones
 *    (Create) and Renames must be acyclic.
 * 4) clone_range sources must not be modified by the same
 *    transaction
 */
class PGTransaction {
public:
  ...
  public:
    ...
    bool has_source() const {
      return match(
	init_type,
	[&](const Init::Create &op) -> bool { return op.clone_source; },
	[&](const Init::Rename &op) -> bool { return true; },
...
private:
  ObjectOperation &get_object_op_for_modify(const hobject_t &hoid) {
    ...
    assert(iter != op_map.end());
    assert(!iter->second.is_delete());
    return iter->second;
  }
public:
  /// Sets up state for existing object 
  void declare_exists(
   ...
   *
   * TODO: once unittesting is in place, there need to be fast paths for the
   * single object case and possibly the single object clone from source
   * case (make_writeable made a clone).
   *
   * Discussion (TODO: remove before merge): This structure only requires that
   * the source->sink graph be acyclic.  This is much more general than is
   * actually required by ReplicatedPG.  Only 4 flavors of multi-object
  ...
  void safe_create_traverse(T &&t) {
	...
	[&](const ObjectOperation::Init::Create &op) {
	  if (op.clone_source)
	    dgraph[*(op.clone_source)].push_back(opair.first);
	},
	[&](const ObjectOperation::Init::Rename &op) {
	  dgraph[op.source].push_back(opair.first);
	},
	[&](const ObjectOperation::Init::None &) {
	},
	[&](const ObjectOperation::Init::Delete &) {
	});
    }
    
    set<hobject_t, hobject_t::BitwiseComparator> done;
    for (auto &&opair: op_map) {
      if (opair.second.has_source()) // not a root
	continue;
       ...
       * before parents.
       *
       * TODO: $5 says this graph traversal is wrong
       */
      list<hobject_t> stack;
      stack.push_front(opair.first);
      while (!stack.empty()) {
