...
public:
  ...
  seastar::future<> read_log_and_missing_crimson(
    ...
    ceph::os::CyanStore::CollectionRef ch,
    const pg_info_t &info,
    ghobject_t pgmeta_oid
    ) {
  ...
  struct CyanStoreLogReader {
    ...
    void process_entry(const pair<std::string, bufferlist> &p) {
      if (p.first[0] == '_')
	return;
      bufferlist bl = p.second;//Copy bufferlist before creating iterator
      auto bp = bl.cbegin();
	...
	[this]() -> seastar::future<std::optional<bool>> {
	...
	}).then([this, reader{std::move(reader)}](bool done) {
          log = IndexedLog(
	     info.last_update,
	     info.log_tail,
	     on_disk_can_rollback_to,
