  ...
  const requested_scrub_t& request_flags)
{
  ...
  dout(10)
    << fmt::format(
	 "{} (from {} {}): {}. <{:.5}>&<{:.10}> --> <{:.5}>&<{:.14}>",
	 __func__, caller, request_flags,
	 (m_listener->sl_is_primary() ? "Primary" : "Replica/other"), pre_reg, pre_state,
...
{
  int grace = m_listener->sl_get_config()->osd_blocked_scrub_grace_period;
  if (grace == 0) {
				    ...
				    bool allow_preemption)
{
  ...
  auto repscrubop = new MOSDRepScrub(spg_t(m_listener->sl_get_info().pgid.pgid, replica.shard),
				     version,
				     m_listener->sl_get_osdmap_epoch(),
				     m_listener->sl_get_last_peering_reset(),
				     start,
				     end,
				     deep,
				     allow_preemption,
				      ...
				      bool deep)
{
  ...
  while (pos.empty()) {
    ...
    pos.deep = deep;
    map.valid_through = m_listener->sl_get_info().last_update;
...
void PgScrubber::handle_scrub_reserve_request(OpRequestRef op)
{
  ...
  } else if (m_listener->sl_get_config()->osd_scrub_during_recovery ||
	     !m_osds->is_recovery_active()) {
...
void PgScrubber::message_all_replicas(int32_t opcode, std::string_view op_text)
{
  ...
  for (auto& p : m_listener->sl_get_actingset()) {
    ...
    dout(10) << "scrub requesting " << op_text << " from osd." << p
	     << " Epoch: " << epch << dendl;
    Message* m = new MOSDScrubReserve(spg_t(m_listener->sl_get_info().pgid.pgid, p.shard),
				      epch,
				      opcode,
				      m_pg_whoami);
...
void PgScrubber::scrub_finish()
{
  ...
  {
    stringstream oss;
    oss << m_listener->sl_get_info().pgid.pgid << " " << m_mode_desc << " ";
    int total_errors = m_shallow_errors + m_deep_errors;
    if (total_errors)
      oss << total_errors << " errors";
    else
      oss << "ok";
    if (!m_is_deep && m_listener->sl_get_info().stats.stats.sum.num_deep_scrub_errors)
      oss << " ( " << m_listener->sl_get_info().stats.stats.sum.num_deep_scrub_errors
  ...
  if (m_is_repair) {
    ...
    if (m_fixed_count == m_shallow_errors + m_deep_errors) {
      ...
      m_listener->sl_queue_scrub_after_recovery();
...
void PgScrubber::set_scrub_begin_time()
{
  ...
  m_osds->clog->debug() << fmt::format(
    "{} {} starts",
    m_listener->sl_get_info().pgid.pgid,
    m_mode_desc);
}
