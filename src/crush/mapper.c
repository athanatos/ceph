 ...
 * Populates *total_tries and *local_tries (if non-null) with last
 * matching rule.
 * @steps: steps to scan
 * @step_len: length of steps
 * @total_tries: out param for CRUSH_RULE_SET_CHOOSE_MSR_TOTAL_TRIES
 * @local_tries: out param for CRUSH_RULE_SET_CHOOSE_MSR_LOCAL_COLLISION_TRIES
 */
...
static unsigned crush_msr_get_retry_value(
	...
	const unsigned local_tries)
{
	const unsigned total_index = (tries * result_max) + index;
	return (total_index << 16) + local_tries;
}
 ...
 * selections that produce a collision (up to local_tries times), but won't
 * retry if it hits an out osd -- that's handled by calling back into
 * crush_msr_choose up to total_tries times.
 *
 * @input: crush input information
	...
	const unsigned tryno)
{
	 ...
	 * step choosemsr 4 host
	 *
	 * guarrantee that the output mapping will be divided into at least
	...
	for (unsigned stride_start = start_index;
		...
		// failed to find non-colliding choice after local_tries attempts
		if (!found) continue;
