...
{
  LOG_PREFIX(OSDSingletoneState::send_pg_temp);
...
{
  LOG_PREFIX(OSDSingletoneState::get_local_map);
...
seastar::future<bufferlist> OSDSingletonState::load_map_bl(
  epoch_t e)
{
  LOG_PREFIX(OSDSingletoneState::load_map_bl);
...
{
  LOG_PREFIX(OSDSingletoneState::store_maps);
  return seastar::do_with(
	...
	return load_map(e - 1).then(
	  [&added_maps, FNAME, e, bl=p->second, &t, this](auto o) {
...
{
  LOG_PREFIX(OSDSingletoneState::trim_maps);
  epoch_t min =
  ...
  epoch_t first)
{
  ...
  INFO("first osdmap: {} superblock's oldest map: {}",
       first, superblock.get_oldest_map());
