...
{
  auto ret = seastar::do_with(PeeringState::PeeringCtx{ctx}, [this](auto& todo) {
    return seastar::when_all_succeed(
      seastar::parallel_for_each(std::move(todo.notify_list),
      ...
      seastar::parallel_for_each(std::move(todo.query_map),
      ...
      seastar::parallel_for_each(std::move(todo.info_map),
        [this](auto& osd_infos) {
