...
void PGMap::PGCreationState::dump_detail(Formatter *f) const final
{
...
std::pair<blocking_future<Ref<PG>>, bool> PGMap::get_pg(spg_t pgid, bool wait)
{
  if (auto pg = pgs.find(pgid); pg != pgs.end()) {
    ...
    return make_pair(
      get_blocking_future(state.promise.get_shared_future()),
      state.creating);
  }
}
