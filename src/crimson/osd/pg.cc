...
std::pair<hobject_t, RWState::State> PG::get_oid_and_lock(MOSDOp &m)
{
  auto oid = m.get_snapid() == CEPH_SNAPDIR ?
    m.get_hobj().get_head() : m.get_hobj();
  #warning fix
  return std::make_pair(oid, RWState::RWWRITE);
}
...
PG::get_or_load_clone_obc(hobject_t oid, SnapSetContextRef ssc)
{
  ...
  using ObjectContextRef = ceph::osd::ObjectContextRef;
  auto coid = resolve_oid(ssc->snapset, oid);
  if (!coid) {
    return seastar::make_ready_future<
      std::pair<ceph::osd::ObjectContextRef, bool>>(
...
seastar::future<ceph::osd::SnapSetContextRef>
PG::get_ssc(const hobject_t &oid)
{
  using SnapSetContextRef = ceph::osd::SnapSetContextRef;
...
seastar::future<ceph::osd::ObjectContextRef>
PG::get_locked_obc(
