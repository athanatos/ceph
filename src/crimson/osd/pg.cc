...
template<class T>
bool compare_set(T& v, const T& new_v)
{
  if (v != new_v) {
    v = new_v;
    return true;
                                ...
                                cached_map_t last_map)
{
  ...
  update_primary_state(new_up, new_up_primary,
                       new_acting, new_acting_primary);
  if (compare_set(info.stats.up, up) +
      compare_set(info.stats.up_primary, up_primary.osd) +
      compare_set(info.stats.acting, acting) +
      compare_set(info.stats.acting_primary, primary.osd)) {
...
void PG::activate(epoch_t activation_epoch)
{
  ...
  if (is_primary()) {
    ...
    send_to_osd(get_primary().osd, m, get_osdmap_epoch());
  }
  // todo:
