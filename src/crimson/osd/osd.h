...
public:
  ...
  auto start_pg_operation(Args&&... args) {
    ...
    auto &opref = *op;
    auto fut = opref.with_blocking_future(
      ...
      logger.debug("{}: start_pg_operation active", opref);
	...
	osdmap_gate.wait_for_map(opref.get_epoch()));
      ...
      logger.debug("{}: start_pg_operation have map", opref);
      return opref.with_blocking_future(
	opref.get_handle().enter(opref.get_connection_pipeline().get_pg));
    }).then([this, &logger, &opref] {
      logger.debug("{}: start_pg_operation in get_pg stage", opref);
