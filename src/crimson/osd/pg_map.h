...
public:
  ...
  /// Returns mapping if present, NULL_MAP otherwise
  mapping_id_t get_pg_mapping(spg_t pgid) {
    auto iter = pg_to_core.find(pgid);
    ceph_assert(iter == pg_to_core.end() || iter->second != NULL_MAP);
    return iter == pg_to_core.end() ? NULL_MAP : iter->second;
  }

  /// Returns mapping for pgid, creates new one if it doesn't already exist
  mapping_id_t maybe_create_pg(spg_t pgid) {
    auto [insert_iter, inserted] = pg_to_core.emplace(pgid, NULL_MAP);
    if (!inserted) {
      ceph_assert(insert_iter->second != NULL_MAP);
  ...
  void remove_pg(spg_t pgid) {
    ...
    ceph_assert(iter->second != NULL_MAP);
  ...
  PGShardMapping(mapping_id_t min_core_mapping, mapping_id_t core_mapping_limit) {
...
private:
  std::map<mapping_id_t, unsigned> core_to_num_pgs;
  std::map<spg_t, mapping_id_t> pg_to_core;
};
