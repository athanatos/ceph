  ...
  instance_handle_t &ihref, Ref<PG> &pg, unsigned this_instance_id)
{
  ...
  return ihref.enter_stage<interruptor>(
    ...
    *this
    ...
    [FNAME, this, pg, this_instance_id]() mutable {
    if (pg->is_primary()) {
      return do_recover_missing(pg, m->get_hobj());
    } else {
      DEBUGDPP("{}.{}: skipping do_recover_missing as this is not the primary",
	       *pg, *this, this_instance_id);
      return interruptor::now();
    }
	  ...
	  return ihref.enter_blocker(
	    *this,
	    ...
	    return pg->with_locked_obc(
	      m->get_hobj(), op_info,
	      [FNAME, this, pg, this_instance_id, &ihref](auto obc) mutable {
