...
public:
   ...
   * BlockingEvent structures before proving that the prior instance can create
   * hangs or crashes due to violations of the BlockerT and PipelineHandle
   * invariants.
   *
  ...
  public:
	...
	typename BlockingEventT::template Trigger<ClientRequest>{
	  std::get<BlockingEventT>(tracking_events), op
	});
