 ...
 * the store was initialized until we add native support to AlienStore,
 * SeaStore, and MemStore.  Once they are adapted, this will likely be
 * removed.
 */
template <typename T>
class ShardedStoreProxy : public FuturizedStore {
  ...
  auto proxy(Method method, Args&&... args) const {
    return crimson::submit_to(
      core,
      [this, method,
       arg_tuple=std::make_tuple(std::forward<Args>(args)...)]() mutable {
	constexpr auto tuple_size = std::tuple_size<decltype(arg_tuple)>::value;
	return apply_method_to_tuple(
	  *impl, method, std::move(arg_tuple),
	  std::make_index_sequence<tuple_size>());
      });
  }

  template <typename Method, typename... Args>
  auto proxy(Method method, Args&&... args) {
    return crimson::submit_to(
      core,
      [this, method,
       arg_tuple=std::make_tuple(std::forward<Args>(args)...)]() mutable {
