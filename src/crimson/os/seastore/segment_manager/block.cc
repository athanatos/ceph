  ...
  bufferptr &bptr)
{
  ...
  return write_ertr::now(
  ...
  }).safe_then([&device, length=bptr.length()](auto result)
	       -> write_ertr::future<> {
    if (result != length) {
      return crimson::ct_error::input_output_error::make();
    }
    return device.flush(
      ...
      [](auto e) -> write_ertr::future<> {
	logger().error(
	  "do_write: flush got error {}",
	  e);
	return crimson::ct_error::input_output_error::make();
      });
  });
...
using open_device_ret = 
  ...
  >;
static
open_device_ret open_device(std::string in_path, seastar::open_flags mode)
{
