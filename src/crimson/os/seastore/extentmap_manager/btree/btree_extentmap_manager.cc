...
BtreeExtentMapManager::initialize_extmap(Transaction &t)
{
  ...
  return tm.alloc_extent<ExtMapLeafNode>(
    t, L_ADDR_MIN, EXTMAP_BLOCK_SIZE
...
BtreeExtentMapManager::find_lextent(
  const extmap_root_t &extmap_root, Transaction &t,
  objaddr_t lo, extent_len_t len)
{
  logger().debug("{}: {}, {}", __func__, lo, len);
  return get_extmap_root(extmap_root, t
  ).safe_then([this, &t, lo, len](auto&& extent) {
...
BtreeExtentMapManager::add_lextent(
  extmap_root_t &extmap_root, Transaction &t,
  objaddr_t lo, lext_map_val_t val)
{
  logger().debug("{}: {}, {}, {}", __func__, lo, val.laddr, val.length);
  return get_extmap_root(extmap_root, t
  ).safe_then([this, &extmap_root, &t, lo, val](auto &&root) {
...
BtreeExtentMapManager::insert_lextent(
  extmap_root_t &extmap_root, Transaction &t,
  ExtMapNodeRef root, objaddr_t logical_offset,
  lext_map_val_t val)
{
  ...
  if (root->at_max_capacity()) {
    ...
    split =  root->extmap_alloc_extent<ExtMapInnerNode>(
      get_ext_context(t), EXTMAP_BLOCK_SIZE
...
BtreeExtentMapManager::rm_lextent(
  ...
  lext_map_val_t val)
{
  ...
  return get_extmap_root(
    extmap_root, t
  ).safe_then([this, &t, lo, val](auto extent) {
