...
struct ExtMapInnerNode
  ...
  find_lextent_ret find_lextent(TransactionManager &tm, Transaction &t, objaddr_t lo, extent_len_t len) final;

  insert_ret insert(TransactionManager &tm, Transaction &t,
                    objaddr_t lo, lext_map_val_t val) final;

  rm_lextent_ret rm_lextent(TransactionManager &tm, Transaction &t, objaddr_t lo, lext_map_val_t val) final;

  split_children_ret make_split_children(TransactionManager &tm, Transaction &t) final;

  full_merge_ret
  make_full_merge(TransactionManager &tm, Transaction &t, ExtMapNodeRef right) final;
  ...
  split_entry_ret split_entry(TransactionManager &tm, Transaction &t, objaddr_t lo,
  ...
  merge_entry_ret merge_entry(TransactionManager &tm, Transaction &t, objaddr_t lo,
...
struct ExtMapLeafNode
 ...
 using internal_iterator_t = const_iterator;
  ...
  insert_ret insert(TransactionManager &tm, Transaction &t, objaddr_t lo, lext_map_val_t val) final;

  rm_lextent_ret rm_lextent(TransactionManager &tm, Transaction &t, objaddr_t lo, lext_map_val_t val) final;

  split_children_ret make_split_children(TransactionManager &tm, Transaction &t) final;

  full_merge_ret make_full_merge(TransactionManager &tm, Transaction &t, ExtMapNodeRef right) final;

  make_balanced_ret make_balanced(TransactionManager &tm, Transaction &t,
