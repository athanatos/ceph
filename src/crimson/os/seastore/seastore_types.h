 ...
 */
class __attribute__((aligned(8), packed)) coll_root_t {
  laddr_le_t coll_root_laddr;
...
public:
  ...
  coll_root_t(laddr_t laddr, segment_off_t size)
    : coll_root_laddr(laddr), size(init_extent_len_le(size)),
      state(state_t::CLEAN) {}
