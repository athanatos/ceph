  ...
  CachedExtentRef extent)
{
  if (extent->is_logical()) {
    ...
    nlbaextent->pin.set_range(nlbaextent->get_node_meta());
     ...
     */
    nlbaextent->resolve_relative_addrs(
      make_record_relative_paddr(0) - nlbaextent->get_paddr());
  ...
  segment_off_t len)
{
  ...
  return cache.get_extent_by_type(
    ...
    return get_root(t).safe_then([=, &t](LBANodeRef root) {
      auto lban = extent->cast<LBANode>();
      return root->lookup(
	op_context_t{cache, pin_set, t},
	lban->get_node_meta().begin,
	lban->get_node_meta().depth).safe_then([=, &t](LBANodeRef c) {
	  if (c->get_paddr() == lban->get_paddr()) {
	    return get_physical_extent_if_live_ret(
	      get_physical_extent_if_live_ertr::ready_future_marker{},
	      lban);
	  } else {
	    cache.drop_from_cache(lban);
	    return get_physical_extent_if_live_ret(
	      get_physical_extent_if_live_ertr::ready_future_marker{},
	      CachedExtentRef());
	  }
