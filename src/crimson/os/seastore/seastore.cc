...
{
  logger().debug("SeaStore::create_new_collection: {}", cid);
...
{
  logger().debug("SeaStore::write_meta, key: {}; value: {}", key, value);
  return seastar::do_with(
    TransactionRef(),
    key,
    value,
    [this](auto &t, auto& key, auto& value) {
      return repeat_eagain([this, &t, &key, &value] {
...
{
  logger().debug("SeaStore::read_meta: key: {}", key);
  return seastar::do_with(
