  ...
  extent_to_write_list_t &to_write)
{
  return crimson::do_for_each(
	...
	return ctx.tm.alloc_extent<TestBlock>(
	  ctx.t,
	  region.addr,
	  region.len
...
using get_ret_bare = std::pair<
  std::optional<extent_to_write_t>,
  std::optional<bufferptr>>;
using get_ret = get_ertr::future<get_ret_bare>;
get_ret get_left(context_t ctx, LBAPinRef &pin, laddr_t offset)
{
  const auto pin_offset = pin->get_laddr();
  assert_aligned(pin_offset);
  ceph_assert(offset >= pin_offset);
  if (offset == pin_offset) {
    return get_ertr::make_ready_future<get_ret_bare>(
      std::nullopt,
      std::nullopt);
    ...
    auto zero_prepend_len = offset - zero_extent_len;
    return get_ertr::make_ready_future<get_ret_bare>(
      (zero_extent_len == 0
       ? std::nullopt
    ...
    auto to_prepend = pin->get_length() - (offset - pin->get_laddr());
...
get_ret get_right(context_t ctx, LBAPinRef &pin, laddr_t end)
{
  ...
  if (end == pin_end) {
    ...
    auto zero_extent_len = (pin->get_laddr() + pin->get_length()) - aligned_end;
    assert_aligned(zero_extent_len);
    return get_ertr::make_ready_future<get_ret_bare>(
      (zero_extent_len == 0
       ? std::nullopt
    ...
    auto to_postpend = pin->get_length() - (pin_end - end);
    return read_pin(ctx, pin->duplicate()
    ).safe_then([to_postpend](auto extent) {
      return get_ertr::make_ready_future<get_ret_bare>(
	std::nullopt,
	bufferptr(extent->get_bptr(), 0, to_postpend));
    });
  }
};
  ...
  lba_pin_list_t &&pins)
{
  return seastar::do_with(
	...
	pins.front(),
	offset
      ).safe_then([this, ctx, &offset, &bl, &pins, &to_write](auto p) {
	auto &[left_extent, headptr] = p;
	if (left_extent) to_write.push_front(std::move(*left_extent));
	if (headptr) {
	  bufferlist newbl;
