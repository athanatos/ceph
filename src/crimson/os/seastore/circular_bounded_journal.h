...
public:
  ...
  void update_applied_to(rbm_abs_addr addr, uint32_t len) {
    ...
    used_size = last_committed_record_base >= new_applied_to ?
		written_to - (new_applied_to + len) :
		written_to + size - (new_applied_to + len);
    applied_to = new_applied_to + len;
  }
  ...
  rbm_abs_addr get_last_committed_record_base() {
    return last_committed_record_base;
  }
