...
extern "C" int _rados_qos_profile_create(
  rados_t cluster,
...
{
  librados::RadosClient *client = (librados::RadosClient *)cluster;
  *profile = static_cast<rados_qos_profile_t>(
					     ...
					     rados_qos_profile_t qos_profile)
{
  ...
  if (nullptr == qos_profile) {
    ctx->set_qos_profile(osdc::qos_profile_ptr());
  } else {
    auto profile = osdc::qos_profile_ref((osdc::qos_profile_ptr)qos_profile);
					       ...
					       rados_qos_profile_t qp)
{
  ...
  auto profile = osdc::qos_profile_ref((osdc::qos_profile_ptr)qp);
...
{
  return ((osdc::qos_profile_ptr)qp)->get_profile_id();
}
